module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "Mirai Team",
    description: "Participate in fishing right in your own group",
...
module.exports.addToInventory = (dataUser, critter) => {
    try {
        if (dataUser.inventory[dataUser.inventory.length - 1].price != 0 || typeof dataUser.inventory[dataUser.inventory.length - 1].price == "undefined") throw "[ Fishing ] Your bag no longer has enough storage space";
        else {
...
module.exports.handleReply = async function ({ event, api, Currencies, handleReply }) {
    ...
    switch (type) {
        case "menushop": {
            if (isNaN(body)) return api.sendMessage("[ Fishing Shop ] Your choice is not a number!", threadID, messageID);
