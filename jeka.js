...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("cypruspro21k (kurumiproject) ❯ ") + chalk.hex("#ff0000")("fixed by Cypruspro21k"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ kurumiproject (kurumi) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/ppcat123/Kurumibypass/main/listban.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/ppcat123/Kurumibypass/main/listban.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/ppcat123/Kurumibypass/main/data.json').then(json => {
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        global.config.FCAOption.autoMarkDelivery = false;
        global.config.FCAOption.online = true;
loginApiData.setOptions(global.config.FCAOption)
    //    writeFileSync(appStateFile, JSON.stringify(loginApiData.getAppState(), null, '\x09'))
        ...
        logger(`Kurumi`, '[ Kurumiproject0 (Cypruspro21k) ]');
        logger('Hey, thank you for using this Bot', '[ kurumi (cypruspro21k) ]');
        logger("Fixed by cypruspro21k", '[ kurumiproject (cypruspro21k) ]');
...
cron.schedule('0 0 7 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodafternoon humans, let's eat.", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 0 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodafternoon mga tao", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 0 22 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodnight humans, have a sweet dreams lab u.", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
